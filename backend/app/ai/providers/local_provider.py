...
from app.ai.token_budget import TokenBudget
...
class GroundedLocalProvider:
    ...
    def _legacy_analyze(self, context: dict) -> dict:
        ...
        evidence_by_node = {
            item["node_id"]: item["evidence_id"] for item in snippets
        }
        edge_by_target = {item["target"]: item for item in edges}

        endpoint_labels = [
            f"{item['http_method']} {item['path']}" for item in endpoints
        ]
        goal_parts = [
        ...
        if len(endpoints) == 1:
            summary = (
                goal_parts[0]
                if goal_parts
                else f"分析接口 {endpoint_labels[0]} 的静态业务链路。"
            )
        else:
            summary = (
        ...
        data_flows = [
            {
                ...
                or "Request",
                "transformations": [
                    f"经过 {node['name']}" for node in ordered_nodes[1:8]
                ],
                "destination": (
                    data_ops[-1]["resource"] if data_ops else "当前调用链返回值"
                ),
                "result": ", ".join(
                    endpoint.get("response_type") or "UNKNOWN"
                    for endpoint in endpoints
                ),
        ...
        impacts = [
            {
                ...
                "node_ids": [
                    node["id"]
                    for node in nodes
                    if endpoint["id"] in node.get("metadata", {}).get(
                        "entryEndpointIds", []
                    )
                ],
                "edge_ids": [],
    ...
    def _business_analyze(self, context: dict) -> dict:
        ...
        if len(endpoints) == 1:
            ...
            business_summary = (
                f"所选 {len(endpoints)} 个接口属于一次联合业务分析。"
                + (
            ...
            business_rules.append(
                {
                    ...
                    "node_ids": list(
                        dict.fromkeys(item["node_id"] for item in group)
                    ),
                }
            )
        ...
        changes = [
            item
            for item in data_ops
            if item["operation"] in {"WRITE", "UPDATE", "DELETE"}
        ]
        flow: list[dict] = []
        entry_node_ids = [
            item["id"] for item in ordered_nodes if item["node_type"] == "API"
        ]
        flow.append(
            {
                "step": 1,
            ...
            flow.append(
                {
                    ...
                    "title": "读取业务信息",
                    "description": (
                        "系统读取完成本次业务判断和结果组装所需的数据。"
                    ),
                    "business_meaning": "使用现有业务数据确认当前情况。",
                    "node_ids": list(
                        dict.fromkeys(item["node_id"] for item in reads)
                    )[:20],
                }
            )
        if changes:
            ...
            flow.append(
                {
                    ...
                    "node_ids": list(
                        dict.fromkeys(item["node_id"] for item in changes)
                    )[:20],
                }
            )
        if external_calls:
            flow.append(
                {
                    ...
                    "node_ids": list(
                        dict.fromkeys(item["node_id"] for item in external_calls)
                    )[:20],
                }
            )
        if len(flow) == 1:
            ...
            key_nodes.append(
                {
                    ...
                    "reason": (
                        f"静态证据显示存在 {item['operation']} 类型的数据操作。"
                    ),
                    "node_ids": [item["node_id"]],
                }
            )
        ...
        external_nodes = [
            item["node_id"]
            for item in external_calls
            if item["dependency_type"] == "EXTERNAL_API"
        ]
            ...
            risks.append(
                {
                    ...
                    "level": "MEDIUM",
                    "node_ids": list(
                        dict.fromkeys(write_nodes + external_nodes)
                    )[:20],
                }
            )
        ...
        source_files = list(
            dict.fromkeys(
                f"{item['file_path']}:{item.get('start_line') or '?'}-"
                f"{item.get('end_line') or '?'}"
                for item in nodes
        ...
        methods = [
            ...
            for item in ordered_nodes
            if item["node_type"]
            in {"ROUTE_FUNCTION", "FUNCTION", "METHOD", "SERVICE"}
        ][:20]
        data_access = [
            item["method_name"]
            for item in data_ops
    ...
    def _business_capability(cls, endpoint: dict) -> str:
        ...
        source = " ".join(
            item
            for item in (module_name.split(".")[-1], function_name)
            if item
        ...
        translated = [
            ...
            for word in words
            if word not in action_map
            and word not in {"common", "api", "v1", "by", "id"}
        ]
