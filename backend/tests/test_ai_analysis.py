...
def test_ai_chain_analysis_is_grounded_and_cached() -> None:
    ...
    with TestClient(app) as client:
        ...
        graph = client.get(
            f"/api/projects/{project['id']}/endpoints/{endpoint['id']}/graph",
            params={"depth": 5},
        ).json()["data"]
...
def test_ai_combined_and_node_impact_analysis() -> None:
    ...
    with TestClient(app) as client:
        ...
        combined = client.post(
            f"/api/projects/{project['id']}/endpoints/ai-combined-analysis",
        ...
        graph = client.get(
            f"/api/projects/{project['id']}/endpoints/{endpoint_ids[0]}/graph",
            params={"depth": 5},
        ).json()["data"]
        node_id = graph["nodes"][0]["id"]
        impact = client.post(
            f"/api/projects/{project['id']}/nodes/{node_id}/ai-impact-analysis",
            json={"depth": 5},
        )
