...
def test_graph_can_expand_and_inspect_evidence() -> None:
    ...
    with TestClient(app) as client:
        ...
        children = client.get(
            f"/api/projects/{project['id']}/nodes/{route_node['id']}/children"
        ).json()["data"]
        ...
        relation = client.get(
            f"/api/projects/{project['id']}/relations/{call_edge['id']}"
        )
        assert relation.status_code == 200
        assert relation.json()["data"]["evidence"]

        source = client.get(
            f"/api/projects/{project['id']}/nodes/{route_node['id']}/source"
        )
        assert source.status_code == 200
        assert "def get_user" in source.json()["data"]["source"]

        combined = client.post(
            f"/api/projects/{project['id']}/graph/combined",
