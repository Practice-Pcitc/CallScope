...
def test_call_graph_resolves_service_repository_and_resources() -> None:
    ...
    nodes_by_qn = {node["qualified_name"]: node for node in result.nodes.values()}
    ...
    assert any(
        relation["relation_type"] == "CALLS" and relation["confidence"] in {"CONFIRMED", "HIGH"}
        for relation in result.relations.values()
    )
    assert any(relation["relation_type"] == "USES_REDIS" for relation in result.relations.values())
    assert any(
...
def test_call_graph_normalizes_imports_below_repository_root(
    ...
    target_key = "method:backend.app.services.user_service.UserService.get_user"
    assert any(
        relation["target_key"] == target_key and relation["confidence"] == "HIGH"
        for relation in result.relations.values()
    )
