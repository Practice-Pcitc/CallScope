...
def test_scan_discovers_python_files_and_ignores_configured_directories(
    ...
    with TestClient(app) as client:
        ...
        start_response = client.post(f"/api/projects/{project_id}/scan")
        assert start_response.status_code == 202

        status_response = client.get(f"/api/projects/{project_id}/scan-status")
...
def test_scan_status_requires_existing_task(tmp_path: Path) -> None:
    with TestClient(app) as client:
        ...
        response = client.get(f"/api/projects/{project['id']}/scan-status")
